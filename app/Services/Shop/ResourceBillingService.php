...
class ResourceBillingService
{
    ...
    public function customConfig(User $user): array
    {
        ...
        $eggs = \Pterodactyl\Models\Egg::query()->whereIn('id', $eggIds ?: [0])->with(['nest:id,name', 'variables'])->get(['id', 'nest_id', 'name'])
            ->map(fn ($egg) => [
                'id' => $egg->id,
                'name' => ($egg->nest?->name ? $egg->nest->name . ' · ' : '') . $egg->name,
                // The variables the client must fill because the egg needs them and gives no default (a FiveM licence...).
                'variables' => $this->requiredVariables($egg),
            ])->values()->all();
    ...
    /**
     * The variables of an egg the client has to fill in: the ones the egg needs (required) and gives no default value for.
     *
     * @return array<int, array{env: string, name: string, description: string}>
     */
    public function requiredVariables(\Pterodactyl\Models\Egg $egg): array
    {
        return $egg->variables
            ->filter(fn ($v) => str_contains((string) $v->rules, 'required')
                && !str_contains((string) $v->rules, 'nullable')
     ...
     * @throws DisplayException
     */
    public function createCustom(User $user, int $eggId, ?int $locationId, array $chosen, string $name, array $variables = [], ?CarbonInterface $now = null): ShopOrder
    {
        if (!$this->settings->customEnabled()) {
        ...
        // The variables the egg needs (a FiveM licence...) must be filled by the client, kept, and given to the server.
        $egg = \Pterodactyl\Models\Egg::query()->with('variables')->findOrFail($eggId);
        $environment = [];
        foreach ($this->requiredVariables($egg) as $variable) {
            $value = trim((string) ($variables[$variable['env']] ?? ''));
            if ($value === '') {
                throw new DisplayException('Please fill in "' . $variable['name'] . '".');
            }
        ...
        try {
            $server = $this->provisioner->provisionCustom($user, $eggId, $fields, $name, $locations, $environment);
        } catch (\Throwable $exception) {
            report($exception);
