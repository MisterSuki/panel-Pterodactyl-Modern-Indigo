...
export interface CustomConfig {
    enabled: boolean;
    eggs: { id: number; name: string; variables: EggVariable[] }[];
...
const mapCustom = (c: any): CustomConfig =>
    c
        ? {
              enabled: !!c.enabled,
              eggs: (c.eggs || []).map((e: any) => ({ id: e.id, name: e.name, variables: e.variables || [] })),
...
// One server bought in the shop, seen from the dashboard: whether it can be changed, if it is a custom (monthly) server,
// and until when it is paid (offers).
...
export const getUpgradeableServers = async (): Promise<UpgradeableServer[]> => {
